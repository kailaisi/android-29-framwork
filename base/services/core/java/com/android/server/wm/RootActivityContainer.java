 ...
 */
//活动容器的根节点。这个类主要是临时的，用来将ActivityStackSupervisor.java中的内容分离出来。意图是将其与RootWindowContainer合并，作为统一层次结构的一部分
class RootActivityContainer extends ConfigurationContainer implements DisplayManager.DisplayListener {
    ...
    //先从给定的显示器id来获取，如果获取不到则直接通过遍历所有的显示器来获取
    ...
    //从mActivityDisplays获取ActivityDisplay，接着不断的从ActivityStack循环寻找和当前ActivityInfo相匹配的ActivityRecord。
    ActivityRecord findActivity(Intent intent, ActivityInfo info, boolean compareIntentFilters) {
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            final ActivityDisplay display = mActivityDisplays.get(displayNdx);
            for (int stackNdx = display.getChildCount() - 1; stackNdx >= 0; --stackNdx) {
                final ActivityStack stack = display.getChildAt(stackNdx);
                final ActivityRecord ar = stack.findActivityLocked(intent, info, compareIntentFilters);
                if (ar != null) {
                    return ar;
                }
            }
    ...
    ActivityRecord isInAnyStack(IBinder token) {
        //mActivityDisplays包含活动的显示列表，按z顺序排序。列表中的最后一项是最上面的
        int numDisplays = mActivityDisplays.size();
        for (int displayNdx = 0; displayNdx < numDisplays; ++displayNdx) {
            final ActivityDisplay display = mActivityDisplays.get(displayNdx);
            for (int stackNdx = display.getChildCount() - 1; stackNdx >= 0; --stackNdx) {
