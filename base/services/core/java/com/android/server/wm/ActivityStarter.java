...
import static com.android.server.wm.ActivityTaskManagerDebugConfig.*;
...
class ActivityStarter {
    ...
    //启动的activity要使用的屏幕id，考虑到分屏？
    private int mPreferredDisplayId;
	//启动的activity使用的栈信息
    private TaskRecord mInTask;
    //标记是否需要将目标Activity添加到TaskRecord中
    ...
    private int startActivity(IApplicationThread caller, Intent intent, Intent ephemeralIntent,
        ...
        //如果有deliverNewIntent权限信息，那么记录日志：   哪个uerid启动通过哪个uid应用启动了intent
        ...
        if (!abort) {
            try {
                Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER,"shouldAbortBackgroundActivityStart");
		...
		//处理PendingActivity的启动，todo 以后处理
    ...
    private int startActivityUnchecked(final ActivityRecord r, ActivityRecord sourceRecord,
        ...
        //根据可复用的ActivityRecord重新结算启动页面的参数信息
        ...
        mRootActivityContainer.sendPowerHintForLaunchStartIfNeeded(false /* forceSend */, mStartActivity);
        //****重点方法*****
        ...
        mSupervisor.handleNonResizableTaskIfNeeded(mStartActivity.getTaskRecord(), preferredWindowingMode, mPreferredDisplayId, mTargetStack);
    ...
    /**
     *
     * @param r   要启动的activity的record
     * @param options
     * @param inTask   源ActivityRecord所处的任务栈
     * @param doResume
     * @param startFlags   启动模式
     * @param sourceRecord  源ActivityRecord
     * @param voiceSession
    ...
    private void setInitialState(ActivityRecord r, ActivityOptions options, TaskRecord inTask,
		...
		//如果启动模式是FLAG_ACTIVITY_NEW_DOCUMENT，而且没有启动的来源，那么将启动模式设置为newTask
        ...
        if ((mLaunchFlags & FLAG_ACTIVITY_NEW_TASK) != 0) {
			//newTask的启动模式下，有一些情况是需要启动多任务的
			//如果设置了FLAG_ACTIVITY_NEW_DOCUMENT和DOCUMENT_LAUNCH_ALWAYS，那么就需要启动多任务
		...
		//如果设置FLAG_ACTIVITY_PREVIOUS_IS_TOP，当前Activity不会作为栈顶来启动新的Activity而是当前Activity的前一个作为栈顶.简而言之，栈ABC启动D则栈变成ABD。
    ...
    private void computeLaunchingTaskFlags() {
        ...
        if (mSourceRecord == null && mInTask != null && mInTask.getStack() != null) {
			//The original intent that started the task.Note that this value can be null.
    ...
    private void computeSourceStack () {
		//通过mSourceRecord获取到调用方的ActivityStack。因为activityRecord持有Acitivity栈信息
        ...
        if ((mLaunchFlags & FLAG_ACTIVITY_NEW_TASK) == 0) {
            ...
            Slog.w(TAG, "startActivity called from finishing " + mSourceRecord+ "; forcing " + "Intent.FLAG_ACTIVITY_NEW_TASK for: " + mIntent);
            mLaunchFlags |= FLAG_ACTIVITY_NEW_TASK;
			//
     ...
     */
     //获取可以重复使用的activity。这种是用来处理singleTop和singleTask这些栈内唯一的启动模式。如果没有，则直接返回空
     ...
     */
    //intentActivity其实是我们的可复用的ActivityRecord
                ...
                if (!willClearTask) {//不需要清空，那么就需要将复用的task移至栈顶
                    ...
                    if (launchStack == null || launchStack == mTargetStack) {
                        ...
                        if ((mLaunchFlags & FLAG_ACTIVITY_LAUNCH_ADJACENT) != 0) {
                            // If we want to launch adjacent and mTargetStack is not the computed launch stack - move task to top of computed stack.
                            intentTask.reparent(launchStack, ON_TOP,REPARENT_MOVE_STACK_TO_FRONT, ANIMATE, DEFER_RESUME,"launchToSide");
                        } else {
                            ...
                            mTargetStack.moveTaskToFrontLocked(intentTask,mNoAnimation, mOptions, mStartActivity.appTimeTracker,"bringToFrontInsteadOfAdjacentLaunch");
                        }
                        mMovedToFront = launchStack != launchStack.getDisplay().getTopStackInWindowingMode(launchStack.getWindowingMode());
                    } else if (launchStack.mDisplayId != mTargetStack.mDisplayId) {
                        //要启动的栈displayId和目标栈的ActivityRecord不一致
                        // Target and computed stacks are on different displays and we've
                        // found a matching task - move the existing instance to that display and
                        // move it to front.
                        intentActivity.getTaskRecord().reparent(launchStack, ON_TOP,REPARENT_MOVE_STACK_TO_FRONT, ANIMATE, DEFER_RESUME,"reparentToDisplay");
                        mMovedToFront = true;
                    } else if (launchStack.isActivityTypeHome() && !mTargetStack.isActivityTypeHome()) {
                        ...
                        intentActivity.getTaskRecord().reparent(launchStack, ON_TOP,REPARENT_MOVE_STACK_TO_FRONT, ANIMATE, DEFER_RESUME,"reparentingHome");
                        mMovedToFront = true;
                    }
                    mOptions = null;
                    ...
                    // delay.
                    intentActivity.showStartingWindow(null /* prev */, false /* newTask */,true /* taskSwitch */);
                }
            }
        }
    ...
    private void setTaskFromIntentActivity(ActivityRecord intentActivity) {
        if ((mLaunchFlags & (FLAG_ACTIVITY_NEW_TASK | FLAG_ACTIVITY_CLEAR_TASK))== (FLAG_ACTIVITY_NEW_TASK | FLAG_ACTIVITY_CLEAR_TASK)) {
            ...
            final ActivityRecord top = intentActivity.getTaskRecord().performClearTaskLocked(mStartActivity, mLaunchFlags);
            if (top == null) {
                // A special case: we need to start the activity because it is not currently
                // running, and the caller has asked to clear the current task to have this
                // activity at the top.
                //因为任务被清空了，所以设置目标的任务栈为空，后面会重新获取任务栈
                mAddingToTask = true;
                // We are no longer placing the activity in the task we previously thought we were.
                mStartActivity.setTask(null);
                // Now pretend like this activity is being started by the top of its task, so it is put in the right place.
                //现在假设activity是由栈顶的任务启动的。
                mSourceRecord = intentActivity;
                final TaskRecord task = mSourceRecord.getTaskRecord();
                if (task != null && task.getStack() == null) {
                    // Target stack got cleared when we all activities were removed above.
                    // Go ahead and reset it.
                    mTargetStack = computeStackFocus(mSourceRecord, false /* newTask */,mLaunchFlags, mOptions);
                    mTargetStack.addTask(task,!mLaunchTaskBehind /* toTop */, "startActivityUnchecked");
                }
            }
            ...
            // 设置mSourceRecord为intentActivity方便后面将activity启动到intentActivity所在的task中。
    ...
    private int setTaskFromSourceRecord() {
        if (mService.getLockTaskController().isLockTaskModeViolation(mSourceRecord.getTaskRecord())) {
            Slog.e(TAG, "Attempted Lock Task Mode violation mStartActivity=" + mStartActivity);
            return START_RETURN_LOCK_TASK_MODE_VIOLATION;
        }
        //获取启动者的任务栈
        ...
        //我们只允许在两种情况下改变堆栈:
        // 1。如果目标任务栈并没有在栈顶。否则，我们将把启动任务移到另一边，而不是同时显示两个任务。
        // 2。如果activity不允许在目标屏幕上显示。
        final int targetDisplayId = mTargetStack != null ? mTargetStack.mDisplayId : sourceStack.mDisplayId;
        //我们在前面已经将要启动的Activity所在的Activity栈移动到前台了，所以sourceStack.topTask就是要启动的Activity所在的栈
        //如果目标Activity不允许在屏幕上显示或者源任务栈和目标任务不再同一个栈（比如说从桌面启动，在Android9中，是在不同的Activity栈的。）
        ...
        if (moveStackAllowed) {
            ...
            //如果仍然没有获取到目标的ActivityStack，或者目标所在屏幕和源数据的屏幕不是同一个，则在源数据的屏幕上找到一个合适的ActivityStack
            ...
            //如果还找不到，就只能在所有的屏幕中找一个合适的ActivityStack了
        ...
        if (!mAddingToTask && (mLaunchFlags & FLAG_ACTIVITY_CLEAR_TOP) != 0) {
            // In this case, we are adding the activity to an existing task, but the caller has asked to clear that task if the activity is already running.
            ActivityRecord top = sourceTask.performClearTaskLocked(mStartActivity, mLaunchFlags);
            mKeepCurTransition = true;
            if (top != null) {//顶部不为空，则调用newIntent方法，然后根据需要调用resume方法
    ...
    private int setTaskFromInTask() {
        ...
        // 在之前进行如果已经将目标Activity放入到任务栈的话，这里就不需要再次放入了，只需要把TaskRecord移动ActivityStack顶部即可
        if (!mAddingToTask) {
            mTargetStack.moveTaskToFrontLocked(mInTask, mNoAnimation, mOptions, mStartActivity.appTimeTracker, "inTaskToFront");
        ...
        if (!mLaunchParams.mBounds.isEmpty()) {
            ...
            ActivityStack stack = mRootActivityContainer.getLaunchStack(null, null, mInTask, ON_TOP);
    ...
    private int setTaskToCurrentTopOrCreateNewTask() {
        ...
        //如果prev为空，而且我们的的acitivity只能后台运行，则报错
    ...
    private ActivityStack getLaunchStack(ActivityRecord r, int launchFlags, TaskRecord task,
        ...
        //当复用Task不为空的时候，直接返回其对应的栈
        if (mReuseTask != null) {
            return mReuseTask.getStack();
        }
		//如果是分屏模式或者当前显示器id和主屏显示器id不一致。那么直接返回分屏所使用的ActivityStack
        if (((launchFlags & FLAG_ACTIVITY_LAUNCH_ADJACENT) == 0)|| mPreferredDisplayId != DEFAULT_DISPLAY) {
            ...
            //****重点方法****
            final ActivityStack stack =mRootActivityContainer.getLaunchStack(r, aOptions, task, onTop, mLaunchParams);
            return stack;
        }
        ...
        if (parentStack != focusedStack) {
            ...
            if (parentStack != null && parentStack.inSplitScreenPrimaryWindowingMode()) {
                ...
                final int activityType =mRootActivityContainer.resolveActivityType(r, mOptions, task);
                return parentStack.getDisplay().getOrCreateStack(WINDOWING_MODE_SPLIT_SCREEN_SECONDARY, activityType, ON_TOP);
            } else {
