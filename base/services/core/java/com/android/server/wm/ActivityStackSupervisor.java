...
public class ActivityStackSupervisor implements RecentTasks.Callbacks {
    ...
    ResolveInfo resolveIntent(Intent intent, String resolvedType, int userId, int flags,
        ...
        try {
            ...
            //清空远程调用端的uid和pid，用当前本地进程的uid和pid替代。token是远程uid和pid的信息
            //这是为了能够通过本地进程的uid和pid来进行当前线程相关操作的调用。
            // 相关调用完成以后然后再将远程uid和pid恢复
            final long token = Binder.clearCallingIdentity();
            try {
                //通过PMS获取ResolveInfo。先调用queryIntentActivities方法出查询相关的列表，然后再调用chooseBestActivity方法去选择
