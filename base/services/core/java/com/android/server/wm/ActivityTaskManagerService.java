...
public class ActivityTaskManagerService extends IActivityTaskManager.Stub {
    ...
    public final int startActivities(IApplicationThread caller, String callingPackage, Intent[] intents, String[] resolvedTypes, IBinder resultTo, Bundle bOptions, int userId) {
        ...
        return getActivityStartController().startActivities(caller, -1, 0, -1, callingPackage, intents, resolvedTypes, resultTo, SafeActivityOptions.fromBundle(bOptions), userId, reason, null /* originatingPendingIntent */,
    ...
    WindowProcessController getProcessController(IApplicationThread thread) {
        ...
        //mProcessNames是一个包装后的map，key：按名称组织。value：保存了当前运行的进程。
        //从缓存获取调用者线程对应的WindowProcessController对象。从队尾开始搜索，肯定是保存的时候将经常使用的放队尾了吧
